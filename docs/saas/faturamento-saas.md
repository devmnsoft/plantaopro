...
## Como testar faturamento SaaS
1. Criar plano ATIVO com limites e flags de mobile/BI/relatórios/integrações.
2. Criar uma assinatura ATIVA por cliente; tentar criar segunda assinatura ativa e confirmar bloqueio.
3. Chamar `POST /api/faturamento-saas/gerar-mensal` para uma competência.
4. Repetir a geração da mesma competência e confirmar que não duplica.
5. Marcar fatura paga exigindo valor positivo, data e forma.
6. Cancelar/contestar exigindo justificativa ou motivo.
7. Consultar `GET /api/faturamento-saas/inadimplencia` após vencimento.
