...
public class PlantoesController(PlantaoService service):ControllerBase{
 ...
 [HttpGet("disponiveis")] public async Task<IActionResult> Disponiveis(){var r=await service.GetAllAsync(); return Ok(ApiResponse<IEnumerable<PlantaoDto>>.Ok(r.Data?.Where(x=>x.Status=="aberto")??Enumerable.Empty<PlantaoDto>(),"Plantões disponíveis."));}
