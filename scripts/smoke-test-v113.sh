...
pass=0; fail=0; rows=()
check(){ local name="$1" method="$2" path="$3" data="${4:-}"; local code rc; set +e; if [ -n "$data" ]; then code=$(curl -sS -o /tmp/v113-smoke.json -w "%{http_code}" -X "$method" "${BASE_URL}${path}" -H 'Content-Type: application/json' -H 'Accept: application/json' "${AUTH[@]}" --data "$data"); rc=$?; else code=$(curl -sS -o /tmp/v113-smoke.json -w "%{http_code}" -X "$method" "${BASE_URL}${path}" -H 'Accept: application/json' "${AUTH[@]}"); rc=$?; fi; set -e; if [ "$rc" -ne 0 ]; then code="000"; fi; if [ "$code" = "200" ] || [ "$code" = "201" ] || [ "$code" = "401" ]; then pass=$((pass+1)); rows+=("{\"name\":\"$name\",\"status\":\"checked\",\"http\":$code}"); else fail=$((fail+1)); rows+=("{\"name\":\"$name\",\"status\":\"failed\",\"http\":$code}"); fi }
