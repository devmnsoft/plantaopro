...
public sealed class FechamentosController : BaseWebController
{
    ...
    [HttpGet("/fechamentos")] public Task<IActionResult> Index(DateOnly? inicio,DateOnly? fim,Guid? unidadeId,Guid? especialidadeId,string? profissional,string? status,CancellationToken ct)=>Load(null,false,ct,inicio,fim,unidadeId,especialidadeId,profissional,status);
    [HttpGet("/fechamentos/pendentes")] public Task<IActionResult> Pendentes(DateOnly? inicio,DateOnly? fim,Guid? unidadeId,Guid? especialidadeId,string? profissional,string? status,CancellationToken ct)=>Load(null,true,ct,inicio,fim,unidadeId,especialidadeId,profissional,status);
    ...
    [HttpGet("/fechamentos/exportar.csv")]
    public async Task<IActionResult> Exportar(DateOnly? inicio,DateOnly? fim,Guid? unidadeId,Guid? especialidadeId,string? profissional,string? status,CancellationToken ct){var client=CreateApiClient();if(!AddBearerToken(client))return HandleUnauthorized();var endpoint=Microsoft.AspNetCore.WebUtilities.QueryHelpers.AddQueryString("api/fechamentos/exportar.csv",new Dictionary<string,string?>{{"inicio",inicio?.ToString("yyyy-MM-dd")},{"fim",fim?.ToString("yyyy-MM-dd")},{"unidadeId",unidadeId?.ToString()},{"especialidadeId",especialidadeId?.ToString()},{"profissional",profissional},{"status",status}});var response=await client.GetAsync(endpoint,ct);if(!response.IsSuccessStatusCode){TempData["Error"]="Não foi possível exportar o relatório com os filtros atuais.";return RedirectToAction(nameof(Index));}return File(await response.Content.ReadAsByteArrayAsync(ct),"text/csv; charset=utf-8",$"apuracao-{DateTime.UtcNow:yyyyMMdd}.csv");}
    private async Task<IActionResult> Load(Guid? id,bool pendentes,CancellationToken ct,DateOnly? inicio=null,DateOnly? fim=null,Guid? unidadeId=null,Guid? especialidadeId=null,string? profissional=null,string? status=null){var client=CreateApiClient();if(!AddBearerToken(client))return HandleUnauthorized();var query=Microsoft.AspNetCore.WebUtilities.QueryHelpers.AddQueryString(pendentes?"api/fechamentos/pendentes":"api/fechamentos",new Dictionary<string,string?>{{"inicio",inicio?.ToString("yyyy-MM-dd")},{"fim",fim?.ToString("yyyy-MM-dd")},{"unidadeId",unidadeId?.ToString()},{"especialidadeId",especialidadeId?.ToString()},{"profissional",profissional},{"status",status}});var list=await ReadApiResponseAsync<IReadOnlyList<FechamentoWebDto>>(client,query);var model=new FechamentoOperacionalPageViewModel{Fechamentos=list.Data??Array.Empty<FechamentoWebDto>(),Error=list.Error,Inicio=inicio,Fim=fim,UnidadeId=unidadeId,EspecialidadeId=especialidadeId,Profissional=profissional??"",Status=status??""};if(id.HasValue){var detail=await ReadApiResponseAsync<FechamentoWebDto>(client,$"api/fechamentos/{id}");var timeline=await ReadApiResponseAsync<IReadOnlyList<FechamentoTimelineWebDto>>(client,$"api/fechamentos/{id}/timeline");model.Selecionado=detail.Data;model.Timeline=timeline.Data??Array.Empty<FechamentoTimelineWebDto>();model.Error??=detail.Error;}return View("~/Views/OperacaoPremium/Fechamentos.cshtml",model);}

    [HttpPost("/fechamentos/{id:guid}/acao"),ValidateAntiForgeryToken]
    public async Task<IActionResult> Acao(Guid id,string acao,string? motivo,string? observacao,CancellationToken ct){var permitidas=new HashSet<string>{"iniciar-conferencia","concluir-conferencia","aprovar","devolver","rejeitar","gerar-financeiro"};if(!permitidas.Contains(acao))return BadRequest();var client=CreateApiClient();if(!AddBearerToken(client))return Unauthorized();object payload=acao switch{"devolver"=>new{Motivo=motivo},"rejeitar"=>new{Motivo=motivo,Observacao=observacao},_=>new{}};var result=await SendApiAsync<object,FechamentoWebDto>(client,HttpMethod.Post,$"api/fechamentos/{id}/{acao}",payload);TempData[result.Data is null?"Error":"Success"]=result.Error??"Ação concluída com sucesso.";return RedirectToAction(nameof(Details),new{id});}
}
