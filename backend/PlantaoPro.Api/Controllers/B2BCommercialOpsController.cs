...
public sealed class MedicoAgendaMeController : ControllerBase
{
    ...
    [HttpPost("disponibilidade")] public async Task<IActionResult> RegistrarDisponibilidade([FromBody] MedicoDisponibilidadeRequest request) { var r = await _service.RegistrarDisponibilidadeAsync(Uid(), request, "DISPONIVEL", Ip()); return StatusCode(r.StatusCode, r); }
    [HttpPost("indisponibilidade")] public async Task<IActionResult> RegistrarIndisponibilidade([FromBody] MedicoDisponibilidadeRequest request) { var r = await _service.RegistrarDisponibilidadeAsync(Uid(), request, "INDISPONIVEL", Ip()); return StatusCode(r.StatusCode, r); }
