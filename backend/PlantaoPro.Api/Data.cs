            ...
            try
            {
                ...
                var escalasRecentes = await cn.ExecuteScalarAsync<int>(@"select count(1)
                    ...
                    where e.medico_id=@m and e.reg_status='A'
                      ...
                      and pl.data_inicio >= (now() - interval '30 days')", new
                {
                    m = medicoId
                }, tx);
...
where id=@plantaoId", new { plantaoId = convite.PlantaoId, userId }, tx);
        ...
        private async Task<ApiResponse<string>> AlterarStatusAsync(Guid id, string novo, string? justificativa, Guid userId, Guid? novoMedicoId, string? ip, string? ua)
        {
            ...
            catch (Exception ex) { await tx.RollbackAsync(); logger.LogError(ex, "Erro status escala {EscalaId}", id); 
                return ApiResponse<string>.Fail("Erro ao alterar status da escala", 500); }
        }
    }

    public sealed class FinanceiroService
    {
        private readonly IConfiguration cfg; private readonly IAuditService audit; private readonly NotificacaoService notificacao; private readonly ICurrentUserService currentUser; private readonly ILogger<FinanceiroService> logger; public FinanceiroService(IConfiguration cfg, IAuditService audit, NotificacaoService notificacao, ICurrentUserService currentUser, ILogger<FinanceiroService> logger)
        {
            this.cfg = cfg;
            this.audit = audit;
            this.notificacao = notificacao;
        ...
        public async Task<ApiResponse<PagamentoActionResponse>> ResolverContestacaoAsync(Guid id, ResolverContestacaoPagamentoRequest req, Guid userId, string? ip, string? ua)
        {
            var decisao=(req.Decisao??string.Empty).Trim().ToUpperInvariant(); var justificativa=(req.Justificativa??string.Empty).Trim();
            if(decisao is not ("MANTER_VALOR" or "AJUSTAR_VALOR" or "CANCELAR_PAGAMENTO"))return ApiResponse<PagamentoActionResponse>.Fail("Decisão inválida.",422);
            if(justificativa.Length<10||justificativa.Length>1000)return ApiResponse<PagamentoActionResponse>.Fail("Justificativa deve possuir entre 10 e 1000 caracteres.",422);
            if(decisao=="AJUSTAR_VALOR"&&(!req.NovoValor.HasValue||req.NovoValor<=0))return ApiResponse<PagamentoActionResponse>.Fail("Novo valor positivo é obrigatório para ajuste.",422);
    ...
    public sealed class MedicoAreaService
    {
        ...
        public MedicoAreaService(IConfiguration cfg, IAuditService audit, ILogger<MedicoAreaService> logger)
        {
            this.cfg = cfg;
            this.audit = audit;
            this.logger = logger;
        ...
        private async Task<(Guid? Id, string? Nome, string? Crm, string? UfCrm)> GetMedicoAsync(NpgsqlConnection cn, Guid uid)
            => await cn.QueryFirstOrDefaultAsync<(Guid?, string?, string?, string?)>("select id,nome,crm,uf_crm from plantaopro.medicos where usuario_id=@uid and reg_status='A' limit 1", new
            {
                uid
            });
        ...
        public async Task<ApiResponse<MedicoAreaResumoDto>> ResumoAsync(Guid uid)
        {
            ...
            if (med.Id is null)
                return ApiResponse<MedicoAreaResumoDto>.Fail("Médico não encontrado para o usuário autenticado.", 404);
            ...
            (select count(1) from plantaopro.notificacoes n where n.usuario_id=@uid and n.lida=false and n.reg_status='A') as NotificacoesNaoLidas", new
            {
                uid,
                mid = med.Id,
                nome = med.Nome,
        ...
        public async Task<ApiResponse<PagedResult<MedicoPlantaoDisponivelDto>>> PlantoesDisponiveisAsync(Guid uid, int page, int pageSize)
        {
            await using var cn = Cn();
            var med = await GetMedicoAsync(cn, uid);
            if (med.Id is null)
                return ApiResponse<PagedResult<MedicoPlantaoDisponivelDto>>.Fail("Médico não encontrado para o usuário autenticado.", 404);
            var p = Math.Max(1, page);
            var s = Math.Clamp(pageSize, 1, 100);
            var off = (p - 1) * s;
            var total = await cn.ExecuteScalarAsync<long>("select count(1) from plantaopro.plantoes p where p.reg_status='A' and p.status='aberto' and p.vagas_disponiveis>0");
...
from plantaopro.plantoes p join plantaopro.hospitais h on h.id=p.hospital_id join plantaopro.especialidades esp on esp.id=p.especialidade_id where p.reg_status='A' and p.status='aberto' and p.vagas_disponiveis>0 order by p.data_inicio asc limit @s offset @off",new{mid=med.Id,s,off});return ApiResponse<PagedResult<MedicoPlantaoDisponivelDto>>.Ok(new(items,p,s,total));}
        ...
        public async Task<ApiResponse<PagedResult<MedicoPagamentoDto>>> MeusPagamentosAsync(Guid uid, int page, int pageSize)
        {
            ...
            coalesce(pg.valor_apurado,pg.valor_previsto) as ValorApurado,pg.valor_aprovado as ValorAprovado,greatest(coalesce(pg.valor_aprovado,pg.valor_apurado,pg.valor_previsto)-coalesce(pg.valor_pago,0),0) as Saldo,
            coalesce(pg.horas_referencia,0) as HorasReferencia,coalesce(pg.valor_hora,0) as ValorHora,pg.escala_id as EscalaId,o.fechamento_id as FechamentoId,f.status as FechamentoStatus,
            (select c.status from plantaopro.pagamento_contestacoes c where c.tenant_id=pg.tenant_id and c.pagamento_id=pg.id order by c.aberto_em desc limit 1) as ContestacaoStatus
            from plantaopro.pagamentos pg
            join plantaopro.plantoes p on p.id=pg.plantao_id
            join plantaopro.hospitais h on h.id=p.hospital_id
            join plantaopro.especialidades esp on esp.id=p.especialidade_id
            left join plantaopro.financeiro_pagamento_origem o on o.tenant_id=pg.tenant_id and o.pagamento_id=pg.id
            left join plantaopro.fechamento_plantao f on f.tenant_id=pg.tenant_id and f.id=o.fechamento_id
            where pg.medico_id=@mid and pg.reg_status='A'
            order by p.data_inicio desc limit @s offset @off", new { mid = med.Id, s, off });
            return ApiResponse<PagedResult<MedicoPagamentoDto>>.Ok(new(items, p, s, total));
        }

        public async Task<ApiResponse<string>> CriarContestacaoAsync(Guid uid, CriarContestacaoPagamentoRequest request, CancellationToken ct)
        {
            var justificativa=(request.Justificativa??string.Empty).Trim();
            if(justificativa.Length<10||justificativa.Length>500)return ApiResponse<string>.Fail("A justificativa deve possuir entre 10 e 500 caracteres.",422);
            await using var cn=Cn();await cn.OpenAsync(ct);await using var tx=await cn.BeginTransactionAsync(ct);
            var pagamento=await cn.QueryFirstOrDefaultAsync<(Guid Id,Guid TenantId,Guid ClienteId,decimal Valor)>(new CommandDefinition(@"select pg.id as Id,pg.tenant_id as TenantId,pg.cliente_id as ClienteId,coalesce(pg.valor_aprovado,pg.valor_apurado,pg.valor_previsto) as Valor from plantaopro.pagamentos pg join plantaopro.medicos m on m.id=pg.medico_id and m.usuario_id=@uid and m.reg_status='A' where pg.id=@PagamentoId and pg.reg_status='A' for update",new{uid,request.PagamentoId},tx,cancellationToken:ct));
            if(pagamento.Id==Guid.Empty)return ApiResponse<string>.Fail("Demonstrativo não encontrado para o profissional autenticado.",404);
            try{await cn.ExecuteAsync(new CommandDefinition(@"insert into plantaopro.pagamento_contestacoes(id,tenant_id,cliente_id,pagamento_id,motivo,status,valor_original,aberto_por) values(gen_random_uuid(),@TenantId,@ClienteId,@Id,@justificativa,'ABERTA',@Valor,@uid)",new{pagamento.TenantId,pagamento.ClienteId,pagamento.Id,pagamento.Valor,justificativa,uid},tx,cancellationToken:ct));await tx.CommitAsync(ct);await audit.LogAsync(uid,"CREATE","pagamento_contestacoes",pagamento.Id,"Divergência do demonstrativo registrada");return ApiResponse<string>.Ok("recebida","Solicitação recebida para análise; nenhum valor foi alterado.");}
            catch(PostgresException ex) when(ex.SqlState==PostgresErrorCodes.UniqueViolation){await tx.RollbackAsync(ct);return ApiResponse<string>.Fail("Já existe uma solicitação ativa para este item.",409);}
        }
    }
