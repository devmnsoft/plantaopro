...
BEGIN
    IF to_regclass('plantaopro.perfis_permissoes') IS NOT NULL THEN
        ALTER TABLE plantaopro.perfis_permissoes
            ADD COLUMN IF NOT EXISTS permitido boolean DEFAULT true,
            ADD COLUMN IF NOT EXISTS bloqueado_por_plano boolean DEFAULT false,
        ...
        INSERT INTO plantaopro.perfil_permissoes(perfil_id,permissao_id,permitido,bloqueado_por_plano,reg_status,reg_date,reg_update,created_by,updated_by)
        SELECT pp.perfil_id, pp.permissao_id, coalesce(pp.permitido,true), coalesce(pp.bloqueado_por_plano,false), coalesce(pp.reg_status,'A'), coalesce(pp.reg_date,now()), pp.reg_update, pp.created_by, pp.updated_by
    ...
    IF to_regclass('plantaopro.usuario_perfis') IS NOT NULL THEN
        ALTER TABLE plantaopro.usuario_perfis
            ADD COLUMN IF NOT EXISTS tenant_id uuid,
            ADD COLUMN IF NOT EXISTS cliente_id uuid,
            ADD COLUMN IF NOT EXISTS reg_status char(1) DEFAULT 'A',
            ADD COLUMN IF NOT EXISTS reg_date timestamptz DEFAULT now(),
            ...
            ADD COLUMN IF NOT EXISTS created_by uuid,
            ADD COLUMN IF NOT EXISTS updated_by uuid;
        INSERT INTO plantaopro.usuarios_perfis(usuario_id,perfil_id,tenant_id,cliente_id,reg_status,reg_date,reg_update,created_by,updated_by)
        SELECT up.usuario_id, up.perfil_id, up.tenant_id, up.cliente_id, coalesce(up.reg_status,'A'), coalesce(up.reg_date,now()), up.reg_update, up.created_by, up.updated_by
