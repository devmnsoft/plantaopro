...
public class NotificacoesController(NotificacaoService service):ControllerBase{
 ...
 [Authorize][HttpGet("nao-lidas")] public async Task<IActionResult> NaoLidas([FromQuery]int page=1,[FromQuery]int pageSize=20){var uid=Guid.Parse(User.Claims.First(c=>c.Type=="uid").Value);var r=await service.ListarAsync(uid,new(null,false,page,pageSize));return StatusCode(r.StatusCode,r);} 
