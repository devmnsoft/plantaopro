...
values(@id,@tenant,@unidade,@plantao,@titulo,@descricao,@categoria,@prioridade,@user)", new { id,tenant=Tenant,unidade=request.UnidadeId,plantao=request.PlantaoId,titulo=request.Titulo.Trim(),descricao=request.Descricao.Trim(),categoria,prioridade,user=UserId },tx,cancellationToken:ct));
    ...
    {
        var destino=Normalize(r.Situacao); await using var cn=Connection(); await cn.OpenAsync(ct); await using var tx=await cn.BeginTransactionAsync(ct);
        ...
        await cn.ExecuteAsync(new CommandDefinition("update plantaopro.ocorrencias_operacionais set situacao=@destino,resolucao=case when @destino='RESOLVIDA' then @descricao else resolucao end,cancelamento_motivo=case when @destino='CANCELADA' then @descricao else cancelamento_motivo end,versao=versao+1,atualizado_em=now() where id=@id and tenant_id=@tenant and versao=@versao",new{id,tenant=Tenant,versao=r.Versao,destino,descricao=r.Descricao?.Trim()},tx,cancellationToken:ct));
