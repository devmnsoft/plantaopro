...
{
    private readonly SaasModuleCatalogService _service;
    private readonly ILogger<ModulosApiController> _logger;
    public ModulosApiController(SaasModuleCatalogService service, ILogger<ModulosApiController> logger) { _service = service; _logger = logger; }
    [HttpGet] public async Task<IActionResult> Listar(CancellationToken ct) { var r = await _service.ListAsync(null, ct); return StatusCode(r.StatusCode, r); }
    [HttpGet("{id:guid}")] public async Task<IActionResult> Obter(Guid id, CancellationToken ct) { var r = await _service.GetAsync(id, null, ct); return StatusCode(r.StatusCode, r); }
    [HttpPost, Authorize(Roles = "ADMINISTRADOR_GLOBAL")] public async Task<IActionResult> Criar([FromBody] SaasModuleUpsertRequest request, CancellationToken ct) { try { var r = await _service.SaveAsync(null, request, Ip(), ct); return StatusCode(r.StatusCode, r); } catch (Exception ex) { _logger.LogError(ex, "Erro ao criar módulo"); return StatusCode(500, ApiResponse<string>.Fail("Não foi possível criar módulo.", 500)); } }
    [HttpPut("{id:guid}"), Authorize(Roles = "ADMINISTRADOR_GLOBAL")] public async Task<IActionResult> Atualizar(Guid id, [FromBody] SaasModuleUpsertRequest request, CancellationToken ct) { try { var r = await _service.SaveAsync(id, request, Ip(), ct); return StatusCode(r.StatusCode, r); } catch (Exception ex) { _logger.LogError(ex, "Erro ao atualizar módulo {Id}", id); return StatusCode(500, ApiResponse<string>.Fail("Não foi possível atualizar módulo.", 500)); } }
    [HttpPost("{id:guid}/habilitar-tenant"), Authorize(Roles = "ADMINISTRADOR_GLOBAL")] public async Task<IActionResult> Habilitar(Guid id, [FromBody] TenantModuleContractRequest request, CancellationToken ct) { try { var r = await _service.ToggleTenantAsync(id, request, true, Ip(), ct); return StatusCode(r.StatusCode, r); } catch (Exception ex) { _logger.LogError(ex, "Erro ao habilitar módulo {Id}", id); return StatusCode(500, ApiResponse<string>.Fail("Não foi possível habilitar módulo.", 500)); } }
    [HttpPost("{id:guid}/desabilitar-tenant"), Authorize(Roles = "ADMINISTRADOR_GLOBAL")] public async Task<IActionResult> Desabilitar(Guid id, [FromBody] TenantModuleContractRequest request, CancellationToken ct) { try { var r = await _service.ToggleTenantAsync(id, request, false, Ip(), ct); return StatusCode(r.StatusCode, r); } catch (Exception ex) { _logger.LogError(ex, "Erro ao desabilitar módulo {Id}", id); return StatusCode(500, ApiResponse<string>.Fail("Não foi possível desabilitar módulo.", 500)); } }
    [HttpPost("{id:guid}/vincular-plano"), Authorize(Roles = "ADMINISTRADOR_GLOBAL")] public async Task<IActionResult> VincularPlano(Guid id, [FromBody] PlanModuleContractRequest request, CancellationToken ct) { try { var r = await _service.LinkPlanAsync(id, request, Ip(), ct); return StatusCode(r.StatusCode, r); } catch (Exception ex) { _logger.LogError(ex, "Erro ao vincular módulo {Id} ao plano", id); return StatusCode(500, ApiResponse<string>.Fail("Não foi possível vincular módulo ao plano.", 500)); } }
    [HttpGet("tenant/{tenantId:guid}")] public async Task<IActionResult> Tenant(Guid tenantId, CancellationToken ct) { var r = await _service.ListAsync(tenantId, ct); return StatusCode(r.StatusCode, r); }
    private string? Ip() => HttpContext.Connection.RemoteIpAddress?.ToString();
}
