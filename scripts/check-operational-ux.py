#!/usr/bin/env python3
"""Gate estático v1.76 das jornadas clínicas e operacionais."""
...
# O fechamento v1.76 exige destinos reais e ausência financeira tratada honestamente.
...
for marker in ("clinicalJourneyClear", "operationalJourneyClear", "actionsWithoutBackendDisabled", "screenshots/v176", "version: '1.76.0'"):
    smoke = (ROOT / "scripts/ui/visual-smoke.mjs").read_text(encoding="utf-8")
    if marker not in smoke:
        errors.append(f"Smoke v1.76 sem contrato operacional: {marker}")

if errors:
    raise SystemExit("Falha na UX operacional v1.76:\n- " + "\n- ".join(errors))
print("UX operacional v1.76 validada: recepção, clínica, operação, drawers e ações reais.")
