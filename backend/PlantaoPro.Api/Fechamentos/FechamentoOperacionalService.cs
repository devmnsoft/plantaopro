...
public sealed class FechamentoOperacionalService
{
    ...
    public async Task<ApiResponse<byte[]>> ExportarCsvAsync(FechamentoFiltroRequest filtro, CancellationToken ct)
    {
        ...
        static string Csv(string value){var safe=value??string.Empty;if(safe.Length>0&&"=+-@".Contains(safe[0]))safe="'"+safe;return "\""+safe.Replace("\"","\"\"")+"\"";}
    ...
    public async Task<ApiResponse<FechamentoDetalheDto>> GerarAsync(Guid plantaoId, string? ip, string? ua, CancellationToken ct)
    {
        ...
        try
        {
            var plantao = await cn.QueryFirstOrDefaultAsync<PlantaoSnapshot>(new CommandDefinition("select p.id as \"Id\",p.hospital_id as \"HospitalId\",p.data_inicio as \"Inicio\",p.data_fim as \"Fim\",p.valor as \"Valor\",p.status as \"Status\" from plantaopro.plantoes p where p.id=@plantaoId and p.tenant_id=@Tenant and p.cliente_id=@Cliente and p.reg_status='A' for update", new { plantaoId, c.Tenant, c.Cliente }, tx, cancellationToken: ct));
            ...
            var escalas = (await cn.QueryAsync<EscalaSnapshot>(new CommandDefinition("select e.id as \"Id\",e.medico_id as \"MedicoId\",e.status as \"Status\",p.data_inicio as \"Inicio\",p.data_fim as \"Fim\",p.valor as \"Valor\" from plantaopro.escalas e join plantaopro.plantoes p on p.id=e.plantao_id where e.plantao_id=@plantaoId and e.tenant_id=@Tenant and e.cliente_id=@Cliente and e.reg_status='A' and e.status in ('realizado','nao_compareceu') and (e.status='nao_compareceu' or exists(select 1 from plantaopro.medico_checkins mc where mc.tenant_id=@Tenant and mc.escala_id=e.id and mc.status_conferencia='APROVADA')) for update", new { plantaoId, c.Tenant, c.Cliente }, tx, cancellationToken: ct))).AsList();
            if (escalas.Count == 0) return ApiResponse<FechamentoDetalheDto>.Fail("O plantão não possui execução conferida elegível; aprove a presença ou registre a ausência antes da apuração.", 422);
