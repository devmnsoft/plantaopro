...
ALTER TABLE IF EXISTS plantaopro.escalas
    ADD CONSTRAINT IF NOT EXISTS ck_escalas_periodo_valido CHECK (data_fim IS NULL OR data_inicio IS NULL OR data_fim > data_inicio),
...
ALTER TABLE IF EXISTS plantaopro.pagamentos
    ...
    ADD CONSTRAINT IF NOT EXISTS ck_pagamentos_horas_referencia_non_negative CHECK (horas_referencia >= 0),
    ADD CONSTRAINT IF NOT EXISTS ck_pagamentos_valor_pago_non_negative CHECK (valor_pago >= 0);
