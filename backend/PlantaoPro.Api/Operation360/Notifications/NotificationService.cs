...
public sealed record NotificationPreferenceDto(string Categoria, bool InApp, bool Email, bool Push);
...
public interface INotificationRepository
{
    ...
    Task<bool> ReadAsync(Guid tenantId, Guid userId, Guid id, CancellationToken ct);
...
public sealed class NotificationRepository : INotificationRepository
{
    ...
    public async Task<bool> ReadAsync(Guid tenantId, Guid userId, Guid id, CancellationToken ct) { await using var cn = Cn(); var sql = @"insert into plantaopro.notification_read_states(id,notification_id,usuario_id,lida_em) select gen_random_uuid(),n.id,@userId,now() from plantaopro.notifications n join plantaopro.notification_recipients r on r.notification_id=n.id and r.usuario_id=@userId where n.id=@id and n.tenant_id=@tenantId and n.reg_status='A' on conflict(notification_id,usuario_id) do nothing"; return await cn.ExecuteAsync(new CommandDefinition(sql, new { tenantId, userId, id }, cancellationToken: ct)) > 0; }
    ...
    public async Task<IReadOnlyList<NotificationPreferenceDto>> PreferencesAsync(Guid tenantId, Guid userId, CancellationToken ct) { await using var cn = Cn(); return (await cn.QueryAsync<NotificationPreferenceDto>(new CommandDefinition("select categoria,in_app as InApp,email,push from plantaopro.notification_preferences where tenant_id=@tenantId and usuario_id=@userId order by categoria", new { tenantId, userId }, cancellationToken: ct))).AsList(); }
    public async Task SavePreferencesAsync(Guid tenantId, Guid userId, IReadOnlyList<NotificationPreferenceDto> prefs, CancellationToken ct) { await using var cn = Cn(); await cn.OpenAsync(ct); await using var tx = await cn.BeginTransactionAsync(ct); foreach (var p in prefs) await cn.ExecuteAsync(new CommandDefinition(@"insert into plantaopro.notification_preferences(id,tenant_id,usuario_id,categoria,in_app,email,push) values(gen_random_uuid(),@tenantId,@userId,@Categoria,@InApp,@Email,@Push) on conflict(tenant_id,usuario_id,categoria) do update set in_app=excluded.in_app,email=excluded.email,push=excluded.push,atualizado_em=now()", new { tenantId, userId, p.Categoria, p.InApp, p.Email, p.Push }, tx, cancellationToken: ct)); await tx.CommitAsync(ct); }
}

public interface IOperationNotificationService { Task<IReadOnlyList<NotificationDto>> ListAsync(bool unread, CancellationToken ct); Task<bool> ReadAsync(Guid id, CancellationToken ct); Task<int> ReadAllAsync(CancellationToken ct); Task<bool> DeleteAsync(Guid id, CancellationToken ct); Task<IReadOnlyList<NotificationPreferenceDto>> PreferencesAsync(CancellationToken ct); Task SavePreferencesAsync(NotificationPreferencesRequest request, CancellationToken ct); }
public sealed class OperationNotificationService : IOperationNotificationService
{
    ...
    public async Task<bool> ReadAsync(Guid id, CancellationToken ct) { var result=await repo.ReadAsync(Tenant,User,id,ct); if(result) await realtime.PublishNotificationAsync(Tenant,User,"NotificacaoLida",new { id },ct); return result; }
