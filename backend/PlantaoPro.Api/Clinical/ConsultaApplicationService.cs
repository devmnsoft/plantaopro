...
public sealed class ConsultaApplicationService : IConsultaApplicationService
{
    ...
    public async Task<ApiResponse<FinalizarConsultaResponse>> FinalizarAsync(Guid id, FinalizarConsultaRequest request, CancellationToken ct)
    {
        ...
        var pendencias = await PendenciasAsync(id, ct);
        if (pendencias.Data is { PodeFinalizar: false }) return ApiResponse<FinalizarConsultaResponse>.Fail("Existem pendências impeditivas.", 422, pendencias.Data.Impeditivas);
        if ((request.TipoFaturamento is TipoFaturamentoAssistencial.CORTESIA or TipoFaturamentoAssistencial.ISENTO) && string.IsNullOrWhiteSpace(request.Justificativa)) return ApiResponse<FinalizarConsultaResponse>.Fail("Cortesia ou isenção exige justificativa.", 400);
        var geraFinanceiro = request.TipoFaturamento is TipoFaturamentoAssistencial.PARTICULAR or TipoFaturamentoAssistencial.CONVENIO or TipoFaturamentoAssistencial.PLANO_SAUDE;
        decimal liquido;
        try { liquido = AtendimentoBillingService.CalcularValorLiquido(request.ValorBruto, request.Desconto, request.Coparticipacao); }
        catch (Exception ex) when (ex is ArgumentOutOfRangeException or InvalidOperationException) { return ApiResponse<FinalizarConsultaResponse>.Fail(ex.Message, 422); }
        if (geraFinanceiro && liquido <= 0) return ApiResponse<FinalizarConsultaResponse>.Fail("Informe um valor líquido real maior que zero para gerar o financeiro.", 422);
        ...
        await Auditar(id, "CONSULTA_FINALIZAR", new { request.Versao, request.TipoFaturamento, ValorLiquido = liquido, FinanceiroId = financeiroId });
        var finalizada = (await repository.ObterAsync(id, tenant, ct))!;
        return ApiResponse<FinalizarConsultaResponse>.Ok(new(finalizada, financeiroId.HasValue, financeiroId, financeiroId.HasValue ? "ABRIR_FATURAMENTO" : "ENCERRAR_ATENDIMENTO"), financeiroId.HasValue ? "Consulta finalizada e financeiro gerado." : "Consulta finalizada sem geração de valor financeiro.");
    }
}
