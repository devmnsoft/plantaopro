...
select
 (select count(1) from plantaopro.v113_clientes where reg_status='A' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))) as clientesAtivos,
 (select count(1) from plantaopro.v113_produtos where reg_status='A' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))) as itensFaturaveisAtivos,
 (select count(1) from plantaopro.v113_tarefas where reg_status='A' and status<>'DONE' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))) as pendenciasOperacionais,
 (select coalesce(sum(valor),0) from plantaopro.v113_faturas where reg_status='A' and status in ('ISSUED','OPEN') and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))) as faturamentoAberto,
 (select count(1) from plantaopro.v113_titulos where reg_status='A' and status in ('OPEN','DEMO_BOLETO') and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))) as titulosAbertos,
 (select count(1) from plantaopro.v113_outbox_eventos where reg_status='A' and status in ('PENDING','ERROR') and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))) as eventosOperacionaisPendentes,
...
from plantaopro.v113_atividades where reg_status='A' order by created_at desc limit 50", "Central de Operação Inteligente 3.0 carregada.");
    ...
    public async Task<ApiResponse<IEnumerable<object>>> TarefasAsync(string? status) => await QueryList("select id,pedido_id as atendimentoOuSolicitacaoId,titulo as titulo,status,responsavel as perfilResponsavel,comentarios as timeline,created_at as criadaEm from plantaopro.v113_tarefas where reg_status='A' and (@status is null or status=@status) and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) order by created_at desc", "Tarefas por perfil carregadas.", ScopeWith(new { status = Normalize(status) }));
    public async Task<ApiResponse<IEnumerable<object>>> OutboxAsync() => await QueryList("select id,tipo as eventoOperacional,payload_ref as referencia,status,erro as erroAmigavel,created_at as criadoEm from plantaopro.v113_outbox_eventos where reg_status='A' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) order by created_at desc", "Eventos operacionais carregados.");
...
from plantaopro.v113_produtos where reg_status <> 'D' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) and (@status is null or status=@status) and (@q is null or nome ilike @like or codigo ilike @like) and (@tipo is null or @tipo in ('CONSULTA','PROCEDIMENTO','PLANTAO','TAXA','REPASSE','CONVENIO','PACOTE')) order by nome", "Itens faturáveis carregados.", ScopeWith(new { q = Blank(q), like = Like(q), tipo = Normalize(tipo), status = Normalize(status) }));
    ...
    public async Task<ApiResponse<ItemFaturavelDto>> AtualizarItemFaturavelAsync(Guid id, ItemFaturavelUpsertDto dto)
    {
        ...
        {
            await using var cn = Cn();
            var affected = await cn.ExecuteAsync("update plantaopro.v113_produtos set codigo=@codigo,nome=@nome,preco=@valor,status=@status,updated_at=now(),updated_by=@userId where id=@id and reg_status <> 'D' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))", new { id, tenantId = TenantId, isGlobal = Global, userId = UserId, codigo = dto.Codigo, nome = dto.Nome, valor = dto.ValorPadrao, status = Normalize(dto.Status) ?? "ACTIVE" });
    ...
    public async Task<ApiResponse<object>> InativarItemFaturavelAsync(Guid id) => await ExecuteCriticalAsync("update plantaopro.v113_produtos set status='INACTIVE',reg_status='D',updated_at=now(),updated_by=@userId where id=@id and reg_status <> 'D' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))", "v114_itens_faturaveis", id, "INATIVAR", "Item faturável inativado.");

    public async Task<ApiResponse<IEnumerable<object>>> ContasReceberAsync() => await QueryList("select id,pedido_id as atendimentoId,valor as valor,status,created_at as emitidaEm from plantaopro.v113_faturas where reg_status='A' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) order by created_at desc", "Contas a receber carregadas.");
    public async Task<ApiResponse<IEnumerable<object>>> TitulosAsync() => await QueryList("select id,fatura_id as contaReceberId,valor,status,demo_boleto as demoBoleto,vencimento as vencimento,'Boleto sempre demonstrativo; sem cobrança real.' as aviso from plantaopro.v113_titulos where reg_status='A' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) order by created_at desc", "Títulos carregados.");
    public async Task<ApiResponse<ContaReceberDto>> GerarContaAtendimentoAsync(Guid atendimentoId) => await faturamentoV115.GerarContaConsultaAsync(atendimentoId, null);
    public async Task<ApiResponse<object>> DemoBoletoAsync(Guid id) => await ExecuteCriticalAsync("update plantaopro.v113_titulos set demo_boleto=true,status='DEMO_BOLETO',updated_at=now(),updated_by=@userId where id=@id and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))", "v114_titulos", id, "DEMO_BOLETO", "Boleto demonstrativo gerado. Nenhuma cobrança real foi emitida.");
    public async Task<ApiResponse<IEnumerable<object>>> RepassesMedicosAsync() => await QueryList("select id,referencia_id as plantaoOuAtendimentoId,valor_repasse as valorRepasse,status,created_at as previstoEm from plantaopro.v115_regras_repasse where reg_status='A' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) order by created_at desc", "Repasses médicos por regra v1.15 carregados.");
    public async Task<ApiResponse<IEnumerable<object>>> GlosasAsync() => await QueryList("select id,conta_receber_id as contaReceberId,valor_glosado as valorGlosado,status,motivo,prazo_recurso as prazoResposta from plantaopro.v115_regras_glosa where reg_status='A' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) order by created_at desc", "Glosas por regra v1.15 carregadas.");
