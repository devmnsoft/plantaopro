...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE plantaopro.escalas
        ADD CONSTRAINT ck_escalas_horas_previstas_positivas
        CHECK (horas_previstas IS NULL OR (horas_previstas > 0 AND horas_previstas <= 24));
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE plantaopro.pagamentos
        ADD CONSTRAINT ck_pagamentos_valor_hora_nao_negativo
        CHECK (valor_hora >= 0);
    END IF;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE plantaopro.pagamentos
        ADD CONSTRAINT ck_pagamentos_horas_referencia_nao_negativa
        CHECK (horas_referencia >= 0);
    END IF;
END $$;
