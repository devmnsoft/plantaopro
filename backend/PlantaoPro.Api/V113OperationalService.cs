...
public sealed class V113OperationalService
{
    ...
    public async Task<ApiResponse<PageResult<CustomerDto>>> ListCustomersAsync(string? q, string? status, int page, int pageSize)
    {
        try
        {
            ...
            var where = "where reg_status <> 'D' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) and (@status is null or status=@status) and (@q is null or nome ilike @like or documento ilike @like or email ilike @like)";
    ...
    public async Task<ApiResponse<CustomerDto>> GetCustomerAsync(Guid id) => await QueryOne<CustomerDto>("select id, nome as name, documento as document, email, status='ACTIVE' as active, created_at as createdAt, updated_at as updatedAt from plantaopro.v113_clientes where id=@id and reg_status <> 'D' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))", new { id, tenantId = TenantId, isGlobal = Global }, "Cliente não encontrado.");
    ...
    public async Task<ApiResponse<CustomerDto>> UpdateCustomerAsync(Guid id, CustomerDto dto)
    {
        ...
        {
            await using var cn = Cn();
            var affected = await cn.ExecuteAsync("update plantaopro.v113_clientes set nome=@name, documento=@document, email=@email, updated_at=now(), updated_by=@userId where id=@id and reg_status <> 'D' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))", new { id, tenantId = TenantId, isGlobal = Global, userId = UserId, dto.Name, dto.Document, dto.Email });
    ...
    public async Task<ApiResponse<PageResult<ProductDto>>> ListProductsAsync(string? q, string? status, bool? critical, int page, int pageSize)
    {
        ...
        {
            await using var cn = Cn();
            var where = "where p.reg_status <> 'D' and (@isGlobal or (@tenantId is not null and p.cliente_id=@tenantId)) and (@status is null or p.status=@status) and (@q is null or p.nome ilike @like or p.codigo ilike @like)";
            ...
            var total = (await cn.QueryAsync<ProductDto>("select x.id, x.code, x.name, x.price, x.minimumStock, x.active, x.balance, x.createdAt, x.updatedAt from (" + baseSql + ") x" + having, args)).Count();
            var rows = await cn.QueryAsync<ProductDto>("select x.id, x.code, x.name, x.price, x.minimumStock, x.active, x.balance, x.createdAt, x.updatedAt from (" + baseSql + ") x" + having + " order by createdAt desc offset @skip limit @take", args);
    ...
    public async Task<ApiResponse<ProductDto>> UpdateProductAsync(Guid id, ProductDto dto)
    {
        ...
        {
            await using var cn = Cn();
            var affected = await cn.ExecuteAsync("update plantaopro.v113_produtos set codigo=@code,nome=@name,preco=@price,estoque_minimo=@minimumStock,updated_at=now(),updated_by=@userId where id=@id and reg_status <> 'D' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))", new { id, tenantId = TenantId, isGlobal = Global, userId = UserId, dto.Code, dto.Name, dto.Price, dto.MinimumStock });
    ...
    public async Task<ApiResponse<InventoryMovementDto>> EntryAsync(InventoryEntryDto dto)
    {
        ...
        {
            var id = Guid.NewGuid(); await using var cn = Cn();
            var product = await cn.ExecuteScalarAsync<int>("select count(1) from plantaopro.v113_produtos where id=@productId and reg_status <> 'D' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))", new { dto.ProductId, tenantId = TenantId, isGlobal = Global });
    ...
    public async Task<ApiResponse<IEnumerable<object>>> MyTasksAsync(string? status) => await QueryList("select id,pedido_id as orderId,titulo as title,status,responsavel as owner,comentarios as comments,created_at as createdAt,updated_at as updatedAt from plantaopro.v113_tarefas where reg_status='A' and (@status is null or status=@status) and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) order by created_at desc", "Tarefas carregadas.", new { tenantId = TenantId, isGlobal = Global, status = Normalize(status) });
    public async Task<ApiResponse<object>> TaskActionAsync(Guid id, string action, CommentDto? comment)
    {
        try { await using var cn = Cn(); var sql = action == "comment" ? "update plantaopro.v113_tarefas set comentarios=coalesce(comentarios,'[]'::jsonb) || to_jsonb(@text::text),updated_at=now(),updated_by=@userId where id=@id" : "update plantaopro.v113_tarefas set status=@status,responsavel=coalesce(responsavel,@owner),updated_at=now(),updated_by=@userId where id=@id"; await cn.ExecuteAsync(sql, new { id, userId = UserId, status = action == "complete" ? "DONE" : "CLAIMED", owner = UserId?.ToString() ?? "homologador", text = comment?.Text ?? string.Empty }); await AuditAsync("v113_tarefas", id, "TASK_" + action.ToUpperInvariant(), true, new { id }); var row = await cn.QueryFirstOrDefaultAsync<object>("select id,pedido_id as orderId,titulo as title,status,responsavel as owner,comentarios as comments,created_at as createdAt,updated_at as updatedAt from plantaopro.v113_tarefas where id=@id", new { id }); return ApiResponse<object>.Ok(row!, "Tarefa atualizada."); } catch (Exception ex) { return Fail<object>(ex, "Falha ao atualizar tarefa."); }
    }

    public async Task<ApiResponse<IEnumerable<object>>> InvoicesAsync() => await QueryList("select id,pedido_id as orderId,valor as amount,status,created_at as createdAt from plantaopro.v113_faturas where reg_status='A' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) order by created_at desc", "Faturas carregadas.");
    public async Task<ApiResponse<IEnumerable<object>>> TitlesAsync() => await QueryList("select id,fatura_id as invoiceId,valor as amount,status,demo_boleto as demoBoleto,vencimento as dueDate from plantaopro.v113_titulos where reg_status='A' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) order by created_at desc", "Títulos carregados.");
    public async Task<ApiResponse<object>> InvoiceFromOrderAsync(Guid orderId) { try { var id = Guid.NewGuid(); await using var cn = Cn(); var amount = await cn.ExecuteScalarAsync<decimal?>("select total from plantaopro.v113_pedidos where id=@orderId and status='CONFIRMED'", new { orderId }); if (!amount.HasValue) return ApiResponse<object>.Fail("Pedido não confirmado.", 400); await cn.ExecuteAsync("insert into plantaopro.v113_faturas(id,cliente_id,tenant_id,pedido_id,valor,status,reg_status,created_at,created_by) values(@id,@tenantId,@tenantId,@orderId,@amount,'ISSUED','A',now(),@userId)", new { id, tenantId = TenantId, userId = UserId, orderId, amount }); await AddOutboxAsync(cn, "INVOICE_ISSUED", id, new { id, orderId }); await AuditAsync("v113_faturas", id, "INVOICE_ISSUED", true, new { orderId }); return ApiResponse<object>.Ok((await cn.QueryFirstAsync<object>("select id,pedido_id as orderId,valor as amount,status,created_at as createdAt from plantaopro.v113_faturas where id=@id", new { id })), "Fatura emitida."); } catch (Exception ex) { return Fail<object>(ex, "Falha ao emitir fatura."); } }
    public async Task<ApiResponse<object>> TitleFromInvoiceAsync(Guid invoiceId) { try { var id = Guid.NewGuid(); await using var cn = Cn(); var amount = await cn.ExecuteScalarAsync<decimal?>("select valor from plantaopro.v113_faturas where id=@invoiceId", new { invoiceId }); if (!amount.HasValue) return ApiResponse<object>.Fail("Fatura não encontrada.", 404); await cn.ExecuteAsync("insert into plantaopro.v113_titulos(id,cliente_id,tenant_id,fatura_id,valor,status,demo_boleto,vencimento,reg_status,created_at,created_by) values(@id,@tenantId,@tenantId,@invoiceId,@amount,'OPEN',false,now()+interval '7 days','A',now(),@userId)", new { id, tenantId = TenantId, userId = UserId, invoiceId, amount }); await AddOutboxAsync(cn, "TITLE_CREATED", id, new { id, invoiceId }); await AuditAsync("v113_titulos", id, "TITLE_CREATED", true, new { invoiceId }); return ApiResponse<object>.Ok((await cn.QueryFirstAsync<object>("select id,fatura_id as invoiceId,valor as amount,status,demo_boleto as demoBoleto,vencimento as dueDate from plantaopro.v113_titulos where id=@id", new { id })), "Título criado."); } catch (Exception ex) { return Fail<object>(ex, "Falha ao criar título."); } }
    ...
    public async Task<ApiResponse<IEnumerable<object>>> OutboxAsync() => await QueryList("select id,tipo as type,payload_ref as payloadRef,status,erro as error,created_at as createdAt from plantaopro.v113_outbox_eventos where reg_status='A' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) order by created_at desc", "Outbox carregada.");
    public async Task<ApiResponse<object>> OutboxActionAsync(Guid id, string action) { try { var status = action == "error" ? "ERROR" : action == "retry" ? "PENDING" : "PROCESSED"; await using var cn = Cn(); await cn.ExecuteAsync("update plantaopro.v113_outbox_eventos set status=@status,erro=case when @status='ERROR' then 'Erro marcado manualmente em homologação' else null end,updated_at=now(),updated_by=@userId where id=@id; insert into plantaopro.v113_outbox_logs(id,cliente_id,tenant_id,outbox_evento_id,status,detalhe,reg_status,created_at,created_by) values(gen_random_uuid(),@tenantId,@tenantId,@id,@status,@detail,'A',now(),@userId)", new { id, status, userId = UserId, tenantId = TenantId, detail = "Ação " + action + " registrada sem envio externo de produção." }); return ApiResponse<object>.Ok((await cn.QueryFirstAsync<object>("select id,tipo as type,payload_ref as payloadRef,status,erro as error,created_at as createdAt,updated_at as updatedAt from plantaopro.v113_outbox_eventos where id=@id", new { id })), "Outbox atualizada."); } catch (Exception ex) { return Fail<object>(ex, "Falha ao atualizar outbox."); } }
    public async Task<ApiResponse<IEnumerable<object>>> OutboxLogsAsync(Guid id) => await QueryList("select id,outbox_evento_id as outboxEventId,status,detalhe as detail,created_at as createdAt from plantaopro.v113_outbox_logs where outbox_evento_id=@id order by created_at desc", "Logs carregados.", new { id, tenantId = TenantId, isGlobal = Global });
    ...
    {
        await using var cn = Cn();
        var orderRows = (await cn.QueryAsync<OrderRow>("select id,cliente_operacional_id as customerId,status,total,created_at as createdAt,updated_at as updatedAt from plantaopro.v113_pedidos where reg_status='A' and (@id is null or id=@id) and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId)) order by created_at desc", new { id, tenantId = TenantId, isGlobal = Global })).ToList();
    ...
    {
        try { await using var cn = Cn(); var affected = await cn.ExecuteAsync("update plantaopro." + table + " set reg_status='D', status='INACTIVE', updated_at=now(), updated_by=@userId where id=@id and reg_status <> 'D' and (@isGlobal or (@tenantId is not null and cliente_id=@tenantId))", new { id, userId = UserId, tenantId = TenantId, isGlobal = Global }); if (affected == 0) return ApiResponse<T>.Fail("Registro não encontrado.", 404); await AuditAsync(table, id, "SOFT_DELETE", true, new { id }); return ApiResponse<T>.Ok(default!, message); } catch (Exception ex) { return Fail<T>(ex, "Falha ao inativar registro."); }
    }
