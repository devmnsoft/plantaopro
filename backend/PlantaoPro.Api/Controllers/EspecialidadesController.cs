...
public class EspecialidadesController : ControllerBase{ private readonly EspecialidadeService service; public EspecialidadesController(EspecialidadeService service){ this.service=service; }
 ...
 [Authorize(Roles = RolesConstants.CadastrosCoordenacao)][HttpGet("{id:guid}")] public async Task<IActionResult> GetById(Guid id){var r=await service.GetByIdAsync(id);return StatusCode(r.StatusCode,r);}
